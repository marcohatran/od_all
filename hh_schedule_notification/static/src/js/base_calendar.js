odoo.define('hh_calendar.hh_calendar', function (require) {
"use strict";
...
var CalendarNotification = Notification.extend({
    template: "HHEventNotification",

    init: function(parent, title, text, eid, action, action_id) {
...
WebClient.include({
    display_alarm_notif: function(notifications) {
        var self = this;
        var last_notif_timer = 0;

        // Clear previously set timeouts and destroy currently displayed calendar notifications
        clearTimeout(this.get_next_alarm_notif_timeout);
        _.each(this.alarm_notif_timeouts, clearTimeout);
        _.each(this.alarm_notif, function(notif) {
            if (!notif.isDestroyed()) {
                notif.destroy();
            }
        });
        this.alarm_notif_timeouts = {};
        this.alarm_notif = {};

        // For each notification, set a timeout to display it
        _.each(notifications, function(notif) {
            self.alarm_notif_timeouts[notif.event_id] = setTimeout(function() {
                var notification = new CalendarNotification(self.notification_manager, notif.title, notif.message, notif.event_id,notif.action,notif.action_id);
                self.notification_manager.display(notification);
                self.alarm_notif[notif.event_id] = notification;
            }, notif.timer * 1000);
        ...
        if (last_notif_timer > 0) {
            this.get_next_alarm_notif_timeout = setTimeout(this.get_next_alarm_notif.bind(this), last_notif_timer * 1000);
        }
    },
    get_next_alarm_notif: function() {
        this.rpc("/alarm/notify", {}, {shadow: true})
            .done(this.display_alarm_notif.bind(this))
    ...
    show_application: function() {
        ...
        bus.on('notification', this, function (notifications) {
            _.each(notifications, (function (notification) {
                if (notification[0][1] === 'hh.alarm') {
                    this.display_alarm_notif(notification[1]);
                }
            }).bind(this));
        });
        return this._super.apply(this, arguments).then(this.get_next_alarm_notif.bind(this));
    },
});
...
//        return this.dataset.call('get_attendee_alarm_detail', [ids, this.getParent().datarecord.id || false])
//                           .then(process_data);
//
...
//});
//
//core.form_widget_registry.add('many2manyattendeealarm', Many2ManyAttendee);
